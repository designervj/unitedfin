...
export function Navbar() {
  ...
  const getLinkClass = (path: string) => {
    return `font-sans font-medium text-[13px] tracking-wider transition-colors uppercase flex items-center group-hover:text-brand-purple ${
      pathname === path ? "text-brand-blue font-semibold" : "text-gray-800"
    }`;
  };

  const getMobileLinkClass = (path: string) => {
    return `block px-3 py-3 text-[14px] font-medium border-b border-gray-100 ${
      pathname === path ? "text-brand-blue" : "text-gray-800"
    }`;
  };

  const renderDropdownMenu = (items: { label: string; href: string }[]) => (
          ...
          <div className="flex justify-between h-[88px] items-center">
            ...
            <div className="hidden lg:flex items-center space-x-2 xl:space-x-4 h-full">
              ...
              <div className="h-full group flex items-center px-3 hover:bg-yellow-100 transition-colors cursor-pointer relative">
                <Link href="/about" className={getLinkClass("/about")}>ABOUT US <ChevronDown className="ml-1 w-4 h-4" /></Link>
                {renderDropdownMenu([
              ...
              <div className="h-full group flex items-center px-3 hover:bg-yellow-100 transition-colors cursor-pointer relative">
                <Link href="/services" className={getLinkClass("/services")}>SERVICES <ChevronDown className="ml-1 w-4 h-4" /></Link>
                {renderDropdownMenu([
              ...
              <div className="h-full group flex items-center px-3 hover:bg-yellow-100 transition-colors cursor-pointer relative">
                <Link href="/emi-calculator" className={getLinkClass("/emi-calculator")}>EMI PAY <ChevronDown className="ml-1 w-4 h-4" /></Link>
                {renderDropdownMenu([
              ...
              <div className="h-full group flex items-center px-3 hover:bg-yellow-100 transition-colors cursor-pointer relative">
                <Link href="/contact" className={getLinkClass("/contact")}>CONTACT <ChevronDown className="ml-1 w-4 h-4" /></Link>
                {renderDropdownMenu([
          ...
          <div className="lg:hidden bg-white border-t border-gray-100 max-h-[80vh] overflow-y-auto">
            <div className="px-4 pt-2 pb-6 space-y-1">
              ...
              <div className="border-b border-gray-100">
                <Link href="/about" onClick={() => setIsOpen(false)} className={getMobileLinkClass("/about").replace('border-b border-gray-100', '')}>ABOUT US</Link>
                <div className="pl-6 pb-2 space-y-2">
                  <Link href="/about" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Company Profile</Link>
                  <Link href="/about#directors" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Board of Directors</Link>
                </div>
              </div>

              <div className="border-b border-gray-100">
                <Link href="/services" onClick={() => setIsOpen(false)} className={getMobileLinkClass("/services").replace('border-b border-gray-100', '')}>SERVICES</Link>
                <div className="pl-6 pb-2 space-y-2">
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Business Loan</Link>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">MSME Loan</Link>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Gold Loan</Link>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Two Wheeler Loan</Link>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Mortgage Loan</Link>
                  <Link href="/services" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Personal Loan</Link>
                </div>
              </div>

              <div className="border-b border-gray-100">
                <Link href="/emi-calculator" onClick={() => setIsOpen(false)} className={getMobileLinkClass("/emi-calculator").replace('border-b border-gray-100', '')}>EMI PAY</Link>
                <div className="pl-6 pb-2 space-y-2">
                  <Link href="/emi-calculator" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">EMI Payment - QR Code</Link>
                  <Link href="/emi-calculator" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">EMI Calculator</Link>
                </div>
              </div>

              <div className="border-b border-gray-100">
                <Link href="/contact" onClick={() => setIsOpen(false)} className={getMobileLinkClass("/contact").replace('border-b border-gray-100', '')}>CONTACT</Link>
                <div className="pl-6 pb-2 space-y-2">
                  <Link href="/contact" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Contact Us</Link>
                  <Link href="/contact" onClick={() => setIsOpen(false)} className="block py-1 text-sm text-gray-600">Careers</Link>
                </div>
              </div>
